      ...
      <table class="finance-table">
        ...
        <tbody>
          <tr v-if="financeStore.expenseItems.length === 0">
            <td colspan="6" class="empty-cell">尚未添加支出項目</td>
          </tr>
        ...
        <tfoot v-if="financeStore.expenseItems.length > 0">
          <tr class="total-row">
            <td colspan="1" class="total-label">年支出總計</td>
            <td colspan="5" class="amount-cell total-amount">{{ financeStore.formatNumber(financeStore.totalExpense) }}</td>
          </tr>
        </tfoot>
      </table>
    </div>
